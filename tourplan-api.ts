...
<Request>
  ...
  async createBooking(params: TourplanBookingParams): Promise<TourplanResponse & { bookingId?: string; bookingReference?: string }> {
    ...
    const bookingReference = `WEB-${timestamp}`
...
<Request>
  <AddServiceRequest>
    ...
    try {
      ...
      if (response.success) {
        console.log('✅ TOURPLAN BOOKING SUCCESS:', response)
        ...
        return {
          ...response,
          bookingId: bookingId || `TPL-${timestamp}`,
          bookingReference: bookingReference
        }
      } else {
        console.log('❌ TOURPLAN BOOKING FAILED:', response.error)
        return response
      }
    } catch (error) {
