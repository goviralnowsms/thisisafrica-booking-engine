...
function applyGroupToursFiltering(products: any[], destination: string, classFilter: string): any[] {
  ...
  // LOG ALL PRODUCTS FROM API RESPONSE
  console.log(`🚌 📦 TourPlan API returned ${products.length} total products:`);
  products.forEach((product, index) => {
    const code = product.code || product.id || 'NO_CODE';
    const location = product.location || 'NO_LOCATION';
    const productClass = product.class || 'NO_CLASS';
    console.log(`🚌   ${index + 1}. ${code} - Location: "${location}" - Class: "${productClass}"`);
  });
...
function checkDestinationMatchFromPatterns(productCode: string, destination: string, destLower: string): boolean {
  ...
  // Botswana destinations
    ...
    return productCode.includes('MUB') || productCode.includes('KAS') || productCode.includes('BBK');
  }
...
function applyDynamicDestinationFiltering(products: any[], destination: string, classFilter: string): any[] {
  ...
  const filteredProducts = products.filter((product: any) => {
    ...
    // PRIMARY: Use API class field if available for precise matching
      ...
      classMatch = productClass.toLowerCase() === classLower;
      if (classMatch) {
        console.log(`✅ ${productCode}: Class match using API field - "${productClass}" matches "${classFilter}"`);
      } else {
        console.log(`❌ ${productCode}: API class "${productClass}" doesn't match "${classFilter}" - NO fallback when API data available`);
      }
    } else {
