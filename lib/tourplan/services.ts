...
// Cruise catalog removed - now using TourPlan API directly
/**
...
// GROUP_TOURS_CATALOG removed - now using API-based filtering directly

// CRUISE_CATALOG removed - now using API-based filtering directly
...
/**
 * Apply Group Tours filtering - Handles all search variations:
 * 1. Country only (returns all tours)
 * 2. Country + Destination (filters by destination)
 * 3. Country + Class (filters by class across all destinations)
 * 4. Country + Destination + Class (filters by both)
 */
function applyGroupToursFiltering(products: any[], destination: string, classFilter: string, productType?: string): any[] {
  const filterLabel = productType?.includes('Packages') ? '📦 Packages' : '🚌 Group Tours';
  console.log(`${filterLabel} Applying API-based filtering`);
  console.log(`${filterLabel} Destination: "${destination}", Class: "${classFilter}"`);
  console.log(`${filterLabel} TourPlan API returned ${products.length} total products`);
  
  // Check if the "destination" is actually just a country name
  const countryNames = ['kenya', 'botswana', 'namibia', 'south africa', 'tanzania', 'zambia', 'zimbabwe', 'uganda', 'rwanda'];
  const isCountryOnlySearch = destination && countryNames.includes(destination.toLowerCase().trim());
  
  // CASE 1: No filters - return all products
  if (!destination && !classFilter) {
    console.log(`${filterLabel} No filters provided, returning all products`);
    return products;
  }
  
  // CASE 2: Country only (no class) - return all products for that country
  if (isCountryOnlySearch && !classFilter) {
    console.log(`${filterLabel} Country-only search: ${destination} - returning all products`);
    // Products are already filtered by country from the API request
    return products;
  }
  
  // CASE 3: Country + Class - filter by class across all destinations
  if (isCountryOnlySearch && classFilter) {
    console.log(`${filterLabel} Country+class search: ${destination} + ${classFilter}`);
    
    // Use dynamic class filtering from API data
    const filteredProducts = products.filter(product => {
      const productClass = (product.class || '').toLowerCase();
      const filterClass = classFilter.toLowerCase();
      
      // More precise class matching - handle exact matches and known variations
      const classMatch = () => {
        // Split product classes by comma/semicolon for multi-class products
        const productClasses = productClass.split(/[,;]/).map(c => c.trim());
        
        for (const pClass of productClasses) {
          // Exact matches
          if (pClass === filterClass) return true;
          
          // Handle "deluxe plus" vs "deluxe" - they should be separate categories
          if (filterClass === 'deluxe' && pClass === 'deluxe plus') return false;
          if (filterClass === 'deluxe plus' && pClass === 'deluxe') return false;
          
          // Handle other variations that should match
          if (filterClass === 'standard' && (pClass === '3 star' || pClass === 'standard plus')) return true;
          if (filterClass === 'deluxe' && (pClass === 'first class' || pClass === 'premium')) return true;
          if (filterClass === 'luxury' && pClass === '5 star') return true;
        }
      ...
      return classMatch() ||
             // Also check ClassDescription field if available
             (product.ClassDescription && product.ClassDescription.toLowerCase().includes(filterClass));
    });
    
    console.log(`${filterLabel} Filtered by class: ${products.length} → ${filteredProducts.length} products`);
    return filteredProducts;
  }
  
  // CASE 4: Specific destination (with or without class)
  // This handles both "Nairobi JKI Airport" and "Nairobi JKI Airport + Deluxe"
  if (!isCountryOnlySearch) {
    console.log(`${filterLabel} Specific destination search: ${destination}${classFilter ? ' + ' + classFilter : ''}`);
  ...
  console.log(`${filterLabel} No specific filtering applied, returning all ${products.length} products`);
  return products;
}

// searchCruisesFromCatalog function removed - now using API-based cruise search directly
...
export async function searchProducts(criteria: {
  ...
  try {
    logger.debug('🔍 Starting product search:', criteria.productType, criteria.destination, criteria.class);
    let xml: string;
    ...
    switch (criteria.productType) {
      ...
      case 'Rail':
      case 'Rail journeys':
        // Use standard search approach for Rail (same as Group Tours/Packages)
        // This will give us location, class, and countries fields from the API
        console.log('🚂 Using standard API search for Rail with destination/class filtering');
        
        // Map rail-specific destinations to countries for the API search
        let railCountryForSearch = '';
        if (criteria.destination) {
          const destLower = criteria.destination.toLowerCase();
          if (destLower.includes('cape town') || destLower.includes('pretoria') || 
              destLower.includes('south africa')) {
            railCountryForSearch = 'South Africa';
          } else if (destLower.includes('victoria falls') || destLower.includes('zimbabwe')) {
            railCountryForSearch = 'Zimbabwe';
          }
        }
        
        if (railCountryForSearch) {
          // Build standard rail search request (will use the default case below)
          xml = buildRailSearchRequest(railCountryForSearch, criteria.dateFrom, criteria.dateTo);
          console.log(`🚂 Rail: Searching country "${railCountryForSearch}" with client-side filtering for destination="${criteria.destination}" class="${criteria.class}"`);
        } else {
          // No valid destination, search all rail products
          xml = buildRailSearchRequest('', criteria.dateFrom, criteria.dateTo);
          console.log('🚂 Rail: Searching all rail products');
        }
        break;
    ...
    // Direct API call without retry logic for faster response
    // Only retry if we get a rate limiting error
    let response;
    try {
      response = await wpXmlRequest(xml);
    } catch (error: any) {
      // Only retry for rate limiting errors
      if (error?.message?.includes('2050') || error?.message?.includes('Request denied')) {
        console.log('🚫 Rate limiting detected, retrying after delay...');
        await delay(2000); // Single 2 second delay
        response = await wpXmlRequest(xml); // One retry attempt
      } else {
    ...
    if (!optionInfo.Option) {
      ...
      // For Cruises, fallback to direct ALL_CRUISES approach if TourPlan API returns empty
      if (criteria.productType === 'Cruises') {
        console.log('🚢 ⚠️ TourPlan API returned no cruise results, falling back to direct cruise product fetch');
        console.log('🚢 Cruise search criteria:', JSON.stringify(criteria, null, 2));
        
        try {
          // Fetch details for all cruise products directly
          const cruiseProducts = [];
          console.log('🚢 Fetching all cruise products:', ALL_CRUISES);
          
          for (const productCode of ALL_CRUISES) {
            try {
              const productDetails = await getProductDetails(productCode);
              ...
              if (productDetails && productDetails.name) {
                ...
                console.log(`✅ Successfully loaded cruise: ${productDetails.name}`);
              }
            } catch (error) {
              console.warn(`⚠️ Failed to fetch cruise ${productCode}:`, error.message);
            }
          }
          
          // Apply class filtering if specified
          let filteredProducts = cruiseProducts;
          if (criteria.class) {
            const classFilter = criteria.class.toLowerCase();
            filteredProducts = cruiseProducts.filter(product => {
              // Filter based on product codes that match class patterns
              const productCode = product.code;
              if (classFilter.includes('standard')) {
                return productCode.includes('TIACP2') || productCode.includes('TIACP3');
              } else if (classFilter.includes('luxury')) {
                // Luxury = Zambezi Queen Standard Suites only (NS products) - WordPress compatible
                return productCode.includes('ZAM2NS') || productCode.includes('ZAM3NS');
              } else if (classFilter.includes('superior')) {
                // Superior = Zambezi Queen Master Suites (NM products)
                return productCode.includes('ZAM2NM') || productCode.includes('ZAM3NM');
              }
              return true; // No specific filter, return all
            });
            console.log(`🚢 Filtered cruises by class ${criteria.class}: ${cruiseProducts.length} → ${filteredProducts.length}`);
          }
          
          console.log(`🚢 Cruise fallback completed: ${filteredProducts.length} cruise products`);
          return {
          ...
          console.error('❌ Cruise fallback failed:', error);
          return {
            products: [],
            totalResults: 0,
            searchCriteria: criteria
      ...
      // For Rail, fallback to catalog approach if TourPlan API returns empty
      if (criteria.productType === 'Rail' || criteria.productType === 'Rail journeys') {
        console.log('🚂 ⚠️ TourPlan API returned no rail results, falling back to curated catalog');
    ...
    const products = await Promise.all(options.map(async (option: any) => {
      ...
      // PERFORMANCE OPTIMIZATION: Group Tours should use fast path (basic data only)
      // Only Rail and Packages need detailed fetching for specific reasons
      if ((criteria.productType === 'Rail' || criteria.productType === 'Rail journeys' || criteria.productType === 'Packages' || criteria.productType === 'Pre-designed packages') && productCode) {
        ...
        if (!hasBasicData) {
          ...
          return {
            ...
            supplier: option.OptGeneral?.SupplierName || '',
            duration: option.OptGeneral?.Periods ? `${option.OptGeneral.Periods} nights` : '',
            location: option.OptGeneral?.LocalityDescription || '', // Use API location field
        ...
        // FAST PATH: For Group Tours and other product types, use basic search response data
        // This avoids slow getProductDetails() API calls and improves performance significantly
        return {
    ...
    const needsDestinationFiltering = criteria.destination && 
                                     (criteria.productType === 'Group Tours' || 
                                      ...
                                      criteria.productType === 'Rail' ||
                                      criteria.productType === 'Rail journeys' ||
                                      criteria.productType === 'Cruises');
    
    // Debug for cruise products specifically
    if (criteria.productType === 'Cruises') {
      console.log('🚢 DEBUG: Cruise API success - TourPlan API is working correctly for cruises');
    ...
    if (needsDestinationFiltering) {
      ...
      if (!criteria.class && !isCountryOnlySearch) {
        ...
        // Use CSV-based filtering for Group Tours AND Packages, WordPress-compatible filtering for Cruises, fall back to dynamic filtering for others
        if (criteria.productType === 'Group Tours' || criteria.productType === 'Guided group tours' ||
            criteria.productType === 'Packages' || criteria.productType === 'Pre-designed packages') {
          const filterType = criteria.productType.includes('Packages') ? 'Packages' : 'Group Tours';
          console.log(`📦 Using CSV-based ${filterType} filtering`);
          console.log(`📦 DEBUG: Before filtering - finalProducts.length = ${finalProducts.length}`);
          console.log(`📦 DEBUG: First 3 products:`, finalProducts.slice(0, 3).map(p => ({
            code: p.code || p.id,
            location: p.location,
            class: p.class,
            countries: p.countries
          })));
          const filteredByGroupTours = applyGroupToursFiltering(finalProducts, criteria.destination, criteria.class, criteria.productType);
          console.log(`📦 DEBUG: After filtering - filteredByGroupTours.length = ${filteredByGroupTours.length}`);
          if (filteredByGroupTours.length !== finalProducts.length) {
            console.log(`📦 Applied ${filterType} CSV filtering: ${finalProducts.length} → ${filteredByGroupTours.length} products`);
            finalProducts = filteredByGroupTours;
          }
        } else if (criteria.productType === 'Cruises' && criteria.class) {
          // Cruise class filtering to match WordPress catalog behavior
          console.log('🚢 Applying WordPress-compatible class filtering for cruises');
          
          const classFilter = criteria.class.toLowerCase();
          const filteredCruises = finalProducts.filter(product => {
            const productCode = product.code || product.id || '';
            
            // Apply WordPress catalog logic for cruise class filtering
            if (classFilter.includes('luxury')) {
              // Luxury = Zambezi Queen Standard Suites only (NS products)
              return productCode.includes('ZAM2NS') || productCode.includes('ZAM3NS');
            } else if (classFilter.includes('superior')) {
              // Superior = Zambezi Queen Master Suites (NM products)
              return productCode.includes('ZAM2NM') || productCode.includes('ZAM3NM');
            } else if (classFilter.includes('standard')) {
              // Standard = Chobe Princess products
              return productCode.includes('TIACP2') || productCode.includes('TIACP3');
            }
          ...
          console.log(`🚢 Filtered cruises by class ${criteria.class}: ${finalProducts.length} → ${filteredCruises.length}`);
          finalProducts = filteredCruises;
        } else {
    ...
    // Cruise filtering moved to main filtering logic above
    } else {
    ...
    console.log(`📊 FINAL RESULT: Returning ${finalProducts.length} products`);
    if (finalProducts.length > 0) {
      console.log(`📊 First product in final result:`, {
        code: finalProducts[0].code || finalProducts[0].id,
    ...
    console.log(`📊 ACTUAL RETURN OBJECT:`, {
      productsLength: resultToReturn.products?.length,
      totalResults: resultToReturn.totalResults,
      hasProducts: !!resultToReturn.products,
      isArray: Array.isArray(resultToReturn.products),
      hasMessage: !!resultToReturn.message,
      hasError: !!resultToReturn.error
    });
...
/**
 * Check if a destination matches using API location field
 */
function checkDestinationMatchFromAPI(apiLocation: string, searchDestination: string): boolean {
  const locationLower = apiLocation.toLowerCase();
  const destLower = searchDestination.toLowerCase();
  
  // Debug logging for MUBGTJENMANJENBSE
  const isDebugProduct = apiLocation.toLowerCase().includes('maun');
  if (isDebugProduct) {
    console.log(`🔍 checkDestinationMatchFromAPI DEBUG:`);
    console.log(`   apiLocation: "${apiLocation}" -> locationLower: "${locationLower}"`);
    console.log(`   searchDestination: "${searchDestination}" -> destLower: "${destLower}"`);
  ...
  if (destLower.includes('rail station') || destLower.includes('rail-station')) {
    ...
    if (destLower.includes('cape town') && locationLower.includes('cape town')) {
      return true;
    }
    if (destLower.includes('pretoria') && locationLower.includes('pretoria')) {
      return true;
    }
    if (destLower.includes('victoria falls') && locationLower.includes('victoria falls')) {
      return true;
    }
    // If no station match found, return false
    return false;
  }
  
  // Special handling for Victoria Falls Town vs other Victoria Falls destinations
  if (destLower.includes('victoria falls town') && locationLower.includes('victoria falls')) {
    return true;
  }
  
  // Handle airport vs city distinctions precisely
  if (destLower.includes('airport')) {
    // For airport searches, location must contain BOTH the city name AND 'airport'
    // e.g., "Johannesburg Airport" should match "Johannesburg Airport" but not "Durban Airport"
    const cityName = destLower.replace(/\s+airport.*$/, '').trim();
    const containsCity = locationLower.includes(cityName);
    const containsAirport = locationLower.includes('airport');
    const result = containsCity && containsAirport;
    if (isDebugProduct) {
      console.log(`   Airport search: cityName = "${cityName}"`);
      console.log(`   locationLower.includes("${cityName}") = ${containsCity}`);
      console.log(`   locationLower.includes('airport') = ${containsAirport}`);
      console.log(`   Result: ${containsCity} && ${containsAirport} = ${result}`);
    }
    return result;
  } else {
    // For city searches, location must contain city name but NOT contain 'airport'
    // This handles cases like "Johannesburg" vs "Johannesburg Airport"
    const cityName = destLower.replace(/\s+airport.*$/, '').trim();
    const containsCity = locationLower.includes(cityName);
    const containsAirport = locationLower.includes('airport');
    const result = containsCity && !containsAirport;
    ...
    if (isDebugProduct) {
      console.log(`   City search: cityName = "${cityName}"`);
      console.log(`   locationLower.includes("${cityName}") = ${containsCity}`);
      console.log(`   locationLower.includes('airport') = ${containsAirport}`);
      console.log(`   Result: ${containsCity} && !${containsAirport} = ${result}`);
    }
...
/**
 * Check if a destination matches using pattern matching (fallback)
 */
function checkDestinationMatchFromPatterns(productCode: string, destination: string, destLower: string): boolean {
  // Kenya destinations
  if (destLower.includes('nairobi') || destLower.includes('jki')) {
    return productCode.includes('NBO');
  }
  // Tanzania destinations  
  else if (destLower.includes('arusha') || destLower.includes('jro') || destLower.includes('kilimanjaro')) {
    return productCode.includes('JRO') || productCode.includes('ARU');
  }
  // Botswana destinations - distinguish between city and airport
  else if (destLower.includes('maun')) {
    // Special handling for Maun Airport vs Maun City
    if (destLower.includes('airport')) {
      // Maun Airport - only match specific airport products
      // Based on CSV: MUBGTJENMANJENBSE is Maun Airport/Deluxe (but Basic class shows 0 results)
      return productCode === 'MUBGTJENMANJENBSE';
    } else {
      // Maun City - match city products (not airport)
      // Based on CSV: MUBGTSUNWAYSUNA13 is Maun/Basic, MUBGTSUNWAYSUBT13 is Maun/Overland Camping
      return productCode === 'MUBGTSUNWAYSUNA13' || productCode === 'MUBGTSUNWAYSUBT13';
    }
  }
  else if (destLower.includes('kasane')) {
    return productCode.includes('KAS') || productCode.includes('BBK');
  }
  // South Africa destinations
  else if (destLower.includes('johannesburg')) {
    return productCode.includes('JNB');
  } else if (destLower.includes('cape town')) {
    return productCode.includes('CPT');
  } else if (destLower.includes('durban')) {
    return productCode.includes('DUR');
  } else if (destLower.includes('port elizabeth')) {
    return productCode.includes('PLZ');
  }
  // Zambia destinations  
  else if (destLower.includes('livingstone')) {
    return productCode.includes('LVI');
  }
  // Zimbabwe destinations
  else if (destLower.includes('victoria falls')) {
    return productCode.includes('VFA');
  }
  // Namibia destinations
  else if (destLower.includes('windhoek')) {
    return productCode.includes('WDH');
  }
  // Rwanda destinations
  else if (destLower.includes('kigali') || destLower.includes('rwanda')) {
    return productCode.includes('KGL');
  }
  // Uganda destinations  
  else if (destLower.includes('entebbe') || destLower.includes('uganda')) {
    return productCode.includes('EBB');
  }
...
function applyDynamicDestinationFiltering(products: any[], destination: string, classFilter: string): any[] {
  ...
  const filteredProducts = products.filter((product: any) => {
    ...
    // PRIMARY: Use API location field if available for precise matching
    if (productLocation && productLocation.trim()) {
      destinationMatch = checkDestinationMatchFromAPI(productLocation, destination);
      if (destinationMatch) {
        console.log(`✅ ${productCode}: Destination match using API location - "${productLocation}" matches "${destination}"`);
      } else {
        console.log(`❌ ${productCode}: API location "${productLocation}" doesn't match "${destination}" - NO fallback when API data available`);
      }
    } else {
      // FALLBACK: Pattern matching ONLY if no API location data available
      destinationMatch = checkDestinationMatchFromPatterns(productCode, destination, destLower);
      if (destinationMatch) {
        console.log(`✅ ${productCode}: Destination match using pattern matching (no API location data)`);
      }
    }
    
    // Note: Removed legacy Botswana special cases - now using API location and class fields for all destinations
    ...
    // CLASS MATCHING - Use API class field first, then fall back to patterns
    ...
    // PRIMARY: Use API class field if available for precise matching
    else if (productClass && productClass.trim()) {
      // Use more precise matching - search class should be contained in product class (not bidirectional)
      const productClassLower = productClass.toLowerCase();
      classMatch = productClassLower.includes(classLower) ||
                   // Specific mappings for known mismatches
                   (classLower === 'deluxe' && (productClassLower === 'first class' || productClassLower === 'premium')) ||
                   (classLower === 'standard' && productClassLower === '3 star') ||
                   (classLower === 'luxury' && productClassLower === '5 star');
      
      if (classMatch) {
        console.log(`✅ ${productCode}: Class match using API field - "${productClass}" contains or maps to "${classFilter}"`);
      } else {
        console.log(`❌ ${productCode}: API class "${productClass}" doesn't match "${classFilter}"`);
      }
    } else {
      // FALLBACK: Pattern matching ONLY if no API class data available
      console.log(`⚠️ ${productCode}: No API class data available, using pattern matching fallback`);
      ...
      if ((classLower === 'basic' || classLower === 'standard') && productCode.includes('RL')) {
